...
def test_n_zero():
    count, groups = max_groups_dfs(0, [])
    assert count == 0
    assert groups == []


def test_n_one():
    count, groups = max_groups_dfs(1, [])
    assert count == 1
    assert sorted(sorted(g) for g in groups) == [[0]]


def test_single_pair():
    count, groups = max_groups_dfs(2, [(0, 1)])
    assert count == 1
...
def test_duplicate_pairs():
    count, groups = max_groups_dfs(3, [(0, 1), (0, 1)])
    assert count == 2
    assert sorted(sorted(g) for g in groups) == [[0, 1], [2]]
...
def test_chain_transitivity():
    count, groups = max_groups_dfs(4, [(0, 1), (1, 2), (2, 3)])
    assert count == 1
    assert sorted(sorted(g) for g in groups) == [[0, 1, 2, 3]]


def test_bridge_merge():
    count, groups = max_groups_dfs(4, [(0, 1), (2, 3), (1, 2)])
    assert count == 1
    assert sorted(sorted(g) for g in groups) == [[0, 1, 2, 3]]
...
def test_n_zero_with_pairs():
    with pytest.raises(ValueError):
        max_groups_dfs(0, [(0, 1)])


def test_pair_out_of_range():
    with pytest.raises(ValueError):
        max_groups_dfs(2, [(2, 3)])


def test_pair_partially_out_of_range():
    with pytest.raises(ValueError):
        max_groups_dfs(3, [(0, 1), (1, 5)])
